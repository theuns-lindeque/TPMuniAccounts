...
import {
  getAppSettings,
  updateAppSettings,
} from "@/app/(main)/actions/settings";
...
const MODELS = [
  {
    id: "gemini-3-flash",
    name: "Gemini 3 Flash",
    description: "Fastest, optimized for interactive conversation.",
    badge: "Speed",
  },
  {
    id: "gemini-3.1-pro",
    name: "Gemini 3.1 Pro",
    description: "Advanced reasoning, best for complex auditing.",
    badge: "Pro",
  },
  {
    id: "gemini-1.5-flash",
    name: "Gemini 1.5 Flash",
    description: "Legacy balanced model (Legacy).",
    badge: "Legacy",
  },
  {
    id: "gemini-1.5-pro",
    name: "Gemini 1.5 Pro",
    description: "Legacy reasoning model (Legacy).",
    badge: "Legacy",
  },
];
...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    if (result.success) {
      ...
      setStatus({
        type: "error",
        message: result.error || "Failed to update protocols.",
      });
    }
    setIsSaving(false);
  };
      ...
      <div className="mb-12">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 uppercase tracking-widest font-mono text-[10px]">
              Node Control & AI Protocol Management
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 relative overflow-hidden">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-6">
              Adjust the artificial intelligence models powering your municipal
              auditing nodes. Higher reasoning models increase accuracy but may
              affect processing latency.
            </p>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <section className="space-y-4">
            ...
            <p className="text-xs text-slate-500 mb-4 font-mono uppercase tracking-tighter">
              powers the utility ai consultant sidebar
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {MODELS.map((model) => (
                <button
                  ...
                  onClick={() =>
                    setSettings((prev) =>
                      prev ? { ...prev, chatModel: model.id } : prev,
                    )
                  }
                  className={`
          ...
          <section className="space-y-4">
            ...
            <p className="text-xs text-slate-500 mb-4 font-mono uppercase tracking-tighter">
              powers background report generation & deep auditing
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {MODELS.map((model) => (
                <button
                  ...
                  onClick={() =>
                    setSettings((prev) =>
                      prev ? { ...prev, analysisModel: model.id } : prev,
                    )
                  }
                  className={`
                    p-4 rounded-xl text-left transition-all border group
