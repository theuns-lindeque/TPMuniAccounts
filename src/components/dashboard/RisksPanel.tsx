    ...
    <div className="border border-slate-200 dark:border-slate-800 rounded-md p-4 h-full bg-white dark:bg-slate-950 overflow-y-auto">
      ...
      <div className="space-y-3">
        {report.anomaliesFound?.map((risk: string, idx: number) => (
          <div
            key={idx}
            className="p-3 border border-slate-100 dark:border-slate-800 rounded bg-slate-50/50 dark:bg-slate-900/50 relative group"
          >
            <p className="text-sm text-slate-700 dark:text-slate-300 pr-20">
              {risk}
            </p>
            <button
      ...
      {selectedRisk && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form
            ...
            className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg shadow-xl max-w-md w-full p-6"
          >
